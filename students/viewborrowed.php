          ...
          <div class="box">
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered">
                ...
                <tbody>
                  <?php
                    $sql = "SELECT *FROM borrow LEFT JOIN students ON students.id=borrow.student_id LEFT JOIN books ON books.id=borrow.book_id WHERE students.id='".$_SESSION['students']."' ORDER BY date_borrow DESC ";
                    $query = $conn->query($sql);
                    while($row = $query->fetch_assoc()){
                      if($row['status'] == 1){
                        $status = '<span class="label label-danger">notreturn</span>';
                      }
                      else{
